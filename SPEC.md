...
12. **2026-06-18 — Sermon Series: a static kit folder with date-ranged kits.** `kit_folders.kind` (`standard` | `sermon_series`) marks one static folder per space, seeded/promoted by name. The Sermon Series folder cannot be renamed or deleted (guarded in server actions and hidden in `KitFolderActions`) and renders its own icon (`BookOpen`) on folder cards. Kits gain a nullable `starts_on`/`ends_on` date range, surfaced only for kits inside the Sermon Series folder: the new-kit and edit-kit dialogs show start/end date pickers when the location is Sermon Series, and the range renders beneath the kit title on cards and the detail page (`formatDateRange`). `createKit` only persists dates when the target folder is the sermon-series kind. The Sermon Series folder is flat — no subfolders (the "New folder" button is hidden on its page and `createKitFolder` rejects a sermon-series parent) — and carries a `BookOpen` icon in the sidebar nav as well as on folder cards. In the sidebar kit tree, dropping a kit onto another kit only reorders within the same folder (it never changes a kit's folder, which previously orphaned kits); moving between folders is done by dropping onto a folder row, with `pointerWithin` collision so drops land on the row under the pointer. Kits inside the Sermon Series folder are ordered by `starts_on` descending (newest series first, undated kits on top, ties by name — `compareSermonSeriesKits`) on both the folder page and the sidebar tree; manual drag-reorder is disabled there because dates own the order.
